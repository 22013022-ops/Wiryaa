...
import { FiAward, FiBriefcase, FiCheckCircle, FiEdit2, FiExternalLink, FiFileText, FiHome, FiImage, FiLink, FiMail, FiMapPin, FiPhone, FiPlus, FiUser, FiUsers } from 'react-icons/fi'
...
function FileDrop({ name, label, hint, accept, icon: Icon, multiple = false }) {
  ...
  const selectFiles = (event) => { const files = event.target.files; setFileName(files?.length > 1 ? `${files.length} files selected` : files?.[0]?.name || '') }
  return <label className="fj-file-drop"><input name={name} type="file" accept={accept} multiple={multiple} onChange={selectFiles} /><span className="fj-file-icon"><Icon /></span><span><strong>{fileName || label}</strong><small>{fileName || hint}</small></span></label>
}
...
function ProfileView({ onEdit }) {
  ...
  return <article className="fj-profile-page">
    <div className="fj-page-title fj-profile-page-title"><div><p>Career workspace</p><h1>My profile</h1><span>A read-only view of the information shared with employers.</span></div><button className="fj-edit-profile" type="button" onClick={() => onEdit(profile)}><FiEdit2 /> Edit Profile</button></div>
