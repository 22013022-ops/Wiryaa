...
function FileDrop({ label, hint, accept, icon: Icon }) {
  const [fileName, setFileName] = useState('')
  return <label className="fj-file-drop"><input type="file" accept={accept} onChange={(event) => setFileName(event.target.files?.[0]?.name || '')} /><span className="fj-file-icon"><Icon /></span><span><strong>{fileName || label}</strong><small>{fileName || hint}</small></span></label>
}
...
function CreateProfile() {
  const { t } = useTranslation(); const [submitted, setSubmitted] = useState(false)
  const handleSubmit = (event) => { if (!event.currentTarget.checkValidity()) return; event.preventDefault(); setSubmitted(true) }
  return <form className="fj-form" onSubmit={handleSubmit}>
